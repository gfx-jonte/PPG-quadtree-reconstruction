...
// @addition J.N
#define KSIZE 7   // AVX vectors are 8 x 32-bit floats so I just use filters of size 7
#define KCENTER 3
#define TILESIZE 32
...
public:
    ...
    SquareMat() {
        matrix.resize(0, std::vector<T>(0));
        dim = 0;
    }
    
    SquareMat(size_t n) {
        matrix.resize(n, std::vector<T>(n));
        dim = n;
    }

    SquareMat(size_t n, T val) {
        matrix.resize(n, std::vector<T>(n, val));
        dim = n;
    }
    ...
    const size_t& size() const { return dim; }
...
public:
    ...
    int computeDepth(const std::vector<QuadTreeNode>& nodes) const {
        ...
        for (int i = 0; i < 4; ++i) {
            if (!isLeaf(i)) {
                maxDepth = std::max(maxDepth, nodes[child(i)].computeDepth(nodes) + 1);
            }
        }
        return maxDepth;
    }
    ...
    // @addition J.N
    void mapToMatrix(SquareMat<Float>& matrix, 
                     SquareMat<char>& mask,     
                     std::vector<QuadTreeNode>& nodes, 
                     int startX, int startY, int quadDim, int tDepth, int depth) {

        const int halfDim = (quadDim / 2); // could also compute this using depth
        for(int c = 0; c < 4; ++c) {
            int sx = (c % 2) == 0 ? startX : startX + halfDim;
            int sy = c > 1 ? startY : startY + halfDim;          // check if this is correct
            
            
            if(isLeaf(c) && (depth < tDepth)) {
                Float v = sum(c) * pow(0.25f, tDepth - depth);
                bool maskPart = pow(2, tDepth - depth) > (KCENTER * 2 + 1); // mask = false if region is larger than kernel
                mapMatrixEntries(v, matrix, mask, maskPart, sx, sy, halfDim);
            }
            else if(depth == tDepth) {
                // TODO: for non-leaves, I think it's worth testing what happens when 0 is mapped
                Float v = isLeaf(c) ? sum(c) : sum(c) / pow(4.f, nodes[child(c)].computeDepth(nodes) + 1);
                mapMatrixEntries(v, matrix, mask, false, sx, sy, halfDim);
            }
            else
                nodes[child(c)].mapToMatrix(matrix, mask, nodes, sx, sy, halfDim, tDepth, depth + 1);
            
        }
    }
    
    // @addition J.N
    void mapToQuadNode(SquareMat<Float>& matrix, std::vector<QuadTreeNode>& nodes,
                     int startX, int startY, int quadDim, int tDepth, int depth) {
        ...
        for(int c = 0; c < 4; ++c) {
            ...
            if(isLeaf(c) || depth == tDepth) {
                for(int i = sy; i < sy + halfDim; ++i) {
                    for(int j = sx; j < sx + halfDim; ++j) {
                        setSum(c, sum(c) + matrix[i][j]);
                    }
                }
            }
            else
                nodes[child(c)].mapToQuadNode(matrix, nodes, sx, sy, halfDim, tDepth, depth + 1);
        }
    }
    
    void rebuildFromLeaves(std::vector<QuadTreeNode>& nodes) {
        for(int c = 0; c < 4; ++c) {
            if(isLeaf(c)) {
                continue;
            }
            nodes[child(c)].rebuildFromLeaves(nodes);
            Float sum = 0.f;
            for(int j = 0; j < 4; ++j) {
                sum += nodes[child(c)].sum(j);
            }
            setSum(c, sum);
        }
    }
...
private:
    ...
    // @addition J.N
    void mapMatrixEntries(Float value, SquareMat<Float>& matrix, SquareMat<char>& mask, 
                          bool shouldMaskAway, int sx, int sy, int halfDim) {
        for(int i = sy; i < sy + halfDim; ++i) {
            for(int j = sx; j < sx + halfDim; ++j) {
                matrix[i + KCENTER][j + KCENTER] = value;

            }
        }
        
        if(shouldMaskAway) {
            for(int i = sy + KCENTER; i < sy + halfDim - KCENTER; ++i) {
                for(int j = sx + KCENTER; j < sx + halfDim - KCENTER; ++j) {
                    mask[i][j] = 0;
                }
            }
        }
...
public:
    ...
    /**
        @addition J.N
        This function denoises the data in a quadtree. The procedure can be
        described as follows:
            - Map the data of the quadtree to matrices in a layer-wise
              manner for each depth in the tree (starting at depth 5)
            - Convolve each matrix with a Gaussian filter
            - Merge the resulting matrices into a single matrix
            - Rebuild the quadtree based on the final matrix
     */
    void denoise(Float sigma, Float subDivisionThreshold) {
        
        // I did some testing and 5 seems like a good cutoff point
        const int denoisingStartDepth = 5;
        
        if(m_maxDepth <= denoisingStartDepth) {
            return;
        }
        
        Float totalFlux = 0;
        for(int c = 0; c < 4; ++c) totalFlux += m_nodes[0].sum(c);
        
        if(totalFlux < Epsilon)
            return;
        
        // Create kernel
        SquareMat<Float> kernel(KSIZE);
        computeKernel(sigma, kernel);

        // Create the layer outputs
        std::vector<std::unique_ptr<SquareMat<Float>>> outputs(m_maxDepth - denoisingStartDepth);
        
        int pos = 0;
        for(int depth = m_maxDepth - 1; depth >= denoisingStartDepth; depth--) {
            SAssert(sigma <= 1.5f && sigma > 0.f); // A sigma outside this range doesn't make sense
            
            int N = 1 << (depth + 1);
            int paddedSize = N + KSIZE - 1;
            SquareMat<Float> input(paddedSize);
            SquareMat<char> mask(N, 1);

            
            // Map node values to input matrix
            m_nodes[0].mapToMatrix(input, mask, m_nodes, 0, 0, N, depth, 0);
            
            
            // Convolve input with filter
            auto output = std::make_unique<SquareMat<Float>>(N);
            convolve2d(input, *output, kernel, mask);
            outputs[pos++] = std::move(output);
            
        }
        
        
        // Reset quadtree data before rebuilding
        for(auto& node : m_nodes)
            node.setSum(0);
        
        
        auto& gather = *std::move(outputs[0]);
        int gSize = gather.size();
        for(int k = 1; k < (int)outputs.size(); ++k) {
            auto& mat = *std::move(outputs[k]);
            int div = gSize / mat.size();
            uint32_t shift;
            asm ( "\tbsr %1, %0\n"
                : "=r"(shift)
                : "r" (div)
            ...
            Float f = 1.f / (float)(div * div);
            __m256 fvec = _mm256_set1_ps(f);
            __attribute__ ((aligned (32))) float inputG[8] = {};
            __attribute__ ((aligned (32))) float inputM[8] = {};
            __attribute__ ((aligned (32))) float out[8] = {};
            
            int pos = 0;
            if(div < 8) {
                for(int i = 0; i < gSize; ++i) {
                    for(int j = 0; j < gSize; j += 8) {
                        pos = 0;
                        for(int x = j; x < j + 8; ++x) {
                            inputG[pos] = gather[i][x];
                            inputM[pos] = mat[i>>shift][x>>shift];
                            pos++;
                        }
                        __m256 vecG = _mm256_load_ps(inputG);
                        __m256 vecM = _mm256_load_ps(inputM);
                        __m256 vecFM = _mm256_mul_ps(vecM, fvec);
                        __m256 result = _mm256_max_ps(vecG, vecFM);
                        _mm256_store_ps(out, result);
                        pos = 0;
                        for(int x = j; x < j + 8; ++x) {
                            gather[i][x] = result[pos++];
                        }
                    }
                }
            }
        ...
        // Normalize the data so that the overall sum is the same as before
        Float totalAfter = 0.f;
        for(int i = 0; i < gSize; ++i) {
#pragma omp simd
            for(int j = 0; j < gSize; ++j) {
                totalAfter += gather[i][j];
            }
        }
        
        Float factor = totalFlux / totalAfter;
        for(int i = 0; i < gSize; ++i) {
            for(int j = 0; j < gSize; ++j) {
                gather[i][j] *= factor;
            }
        ...
        // Refine the structure if needed
        refine(gather, gSize, m_maxDepth-1, subDivisionThreshold, totalFlux);        
        // Map the data back to the quadtree leaves
        m_nodes[0].mapToQuadNode(gather, m_nodes, 0, 0, gSize, m_maxDepth-1, 0);
        // Rebuild maintain the property that each parent is the sum of the children
        m_nodes[0].rebuildFromLeaves(m_nodes);
        
        // Update overall sum of quadtree
        Float sum = 0.f;
        for(int c = 0; c < 4; ++c) sum += m_nodes[0].sum(c); 
        m_atomic.sum.store(sum);
...
private:
    ...
    // @addition J.N
    Float sumOfRegion(const SquareMat<Float>& matrix, int sx, int sy, int dim) {
        Float sum = 0.f;
        for(int i = sy; i < sy + dim; ++i) {
            for(int j = sx; j < sx + dim; ++j) {
                sum += matrix[i][j];
            }
        }
        return sum;
    ...
    /* @addition J.N
     * This function refines the structure of the quadtree according to the
     * denoised matrices. It basically follows the same subdivision scheme 
     * proposed by Müller
     */
    void refine(const SquareMat<Float>& matrix, int rootDim, int tDepth, Float subDivisionThreshold, Float totalFlux) {
        struct StackNode {
            size_t nodeIndex;
            int sx;
            int sy;
        ...
        std::stack<StackNode> nodeIndices;
        nodeIndices.push({0, 0, 0, 0, rootDim / 2});
        
        while(!nodeIndices.empty()) {
            StackNode sNode = nodeIndices.top();
            nodeIndices.pop();
            
            if(sNode.depth < tDepth) {
                for(int c = 0; c < 4; ++c) {
                    int sx = (c % 2) == 0 ? sNode.sx : sNode.sx + sNode.childDim;
                    int sy = c > 1 ? sNode.sy : sNode.sy + sNode.childDim;
                
                    if(m_nodes[sNode.nodeIndex].isLeaf(c)) {
                        Float leafSum = sumOfRegion(matrix, sx, sy, sNode.childDim);
                        Float fraction = leafSum / totalFlux;
                        if(fraction > subDivisionThreshold * 0.5f) {
                            nodeIndices.push({m_nodes.size(), sx, sy, sNode.depth + 1, sNode.childDim / 2});
                            m_nodes[sNode.nodeIndex].setChild(c, static_cast<uint16_t>(m_nodes.size()));
                            m_nodes.emplace_back();
                    
                            if (m_nodes.size() >= std::numeric_limits<uint16_t>::max()) {
                                cout << "DTree hit maximum children count!" << endl;
                                nodeIndices = std::stack<StackNode>();
                                break;
                            }
                        }
                        ...
                        nodeIndices.push({m_nodes[sNode.nodeIndex].child(c), sx, sy, sNode.depth + 1, sNode.childDim / 2});
                    }
                }
            }
        }
    ...
    // @addition J.N
    void computeKernel(Float sigma, SquareMat<Float>& kernel) {
        double p, q = 2.0 * sigma * sigma;
        double sum = 0.0;
        for (int x = -KCENTER; x <= KCENTER; x++) {
            for (int y = -KCENTER; y <= KCENTER; y++) {
                p = sqrt(x * x + y * y);
                kernel[x + KCENTER][y + KCENTER] = (exp(-(p * p) / q)) / (M_PI * q);
                sum += kernel[x + KCENTER][y + KCENTER];
            }
        }
    ...
    /* @addition J.N
     * Tiled matrix convoulution using AVX
     */
    void convolve2d(const SquareMat<Float>& input,
                    SquareMat<Float>& output,
                    const SquareMat<Float>& kernel,
                    const SquareMat<char>& mask) {
        
        const int N = input.size(); //  I will assume N*N matrix
        __m256 kernelVectors[7];
        for(int i = 0; i < KSIZE; ++i) {
            __attribute__ ((aligned (32))) float kVecRow[8] = { };
            for(int j = 0; j < KSIZE; ++j)
                kVecRow[j] = kernel[i][j];
            kernelVectors[i] = _mm256_load_ps(kVecRow);
        }

        /* CAUTION: this loop could break for inappropriately sized matrices.
        * The assumption is that the matrix is square and the dimension is a power of 2 (+padding)
        */
        for(int i = KCENTER; i < N - KCENTER; i += TILESIZE) {                      // rows
            for(int it = i; it < i + TILESIZE; ++it) {

                for(int j = KCENTER; j < N - KCENTER; j += TILESIZE) {              // columns
                    for(int jt = j; jt < j + TILESIZE; ++jt) {
                
                        if(!mask[it - KCENTER][jt - KCENTER]) {                     // Check mask
                            output[it - KCENTER][jt - KCENTER] = input[it][jt];
                            continue;
                        }

                        __attribute__ ((aligned (32))) float inputV[8] = { };
                        __m256 inputVectors[7];

                        int rowIndex = KSIZE - 1;
                        for(int m = it - KCENTER; m <= it + KCENTER; ++m) {
                            int vectorIndex = KSIZE - 1;
                            for(int n = jt - KCENTER; n <= jt + KCENTER; ++n) {
                                inputV[vectorIndex--] = input[m][n];
                            }
                            inputVectors[rowIndex--] = _mm256_load_ps(inputV);
                        }

                        // my i7 3770k does not support fused multiply add unfortunately
                        __m256 sumV = _mm256_setzero_ps();
                        for(int k = 0; k < KSIZE; ++k) {
                            __m256 prod = _mm256_mul_ps(inputVectors[k], kernelVectors[k]);
                            sumV = _mm256_add_ps(prod, sumV);
                        }

                        __attribute__ ((aligned (32))) float gather[8] = { };
                        _mm256_store_ps(gather, sumV);
                        float sum = 0.f;
                        for(int k = 0; k < KSIZE; ++k) {
                            sum += gather[k];
                        }
                        output[it - KCENTER][jt - KCENTER] = sum;
                    }
                }
            }
        }
...
public:
    ...
    void denoise(Float sigma, Float threshold) {
        sampling.denoise(sigma, threshold);
    }
...
public:
    GuidedPathTracer(const Properties &props) : MonteCarloIntegrator(props) {
        ...
        } else if (m_distributionStr == "switch") {
            m_distribution = EDistribution::ESimple;
        } else {
            Assert(false);
        }
        ...
        m_denoising = props.getBoolean("denoising", false);
        m_sigma = props.getFloat("dTreeSigma", 0.8f);
        m_recWL = props.getBoolean("recWL", true);
    }
    ...
    void buildSDTree(bool doBuild = true) {
        ...
        unsigned long long treeWeight = 0;
        const int denoisingMaxDepth = 10;
        ...
        m_sdTree->forEachDTreeWrapperConst([&](const DTreeWrapper* dTree) {
            ...
            if(depth >= 6 && depth <= denoisingMaxDepth)
                treeWeight += (1 << depth) * (1 << depth);
        ...
        /* @addition J.N
         * Workload limiting
         * TODO: Might want to represent workload in a different way since overflow could happen
         */
        if(doBuild && m_denoising) {
            auto start = std::chrono::steady_clock::now();
            if(m_recWL) {
                unsigned long long baseDenoisingThreshold = 15000000;
                unsigned long long threshold = baseDenoisingThreshold * (1<<m_iter);
                if(treeWeight < threshold) {
                    m_sdTree->forEachDTreeWrapperParallel([this](DTreeWrapper* dTree) {
                            if(dTree->depth() <= denoisingMaxDepth)
                                dTree->denoise(m_sigma, m_dTreeThreshold);
                        });
                
                    Log(EInfo, "DENOISING QUADTREES TOOK %f SECONDS", computeElapsedSeconds(start));
                }
                else
                    Log(EInfo, "TOO MUCH WORK TO DENOISE");
            }
            else {
                m_sdTree->forEachDTreeWrapperParallel([this](DTreeWrapper* dTree) {
                        dTree->denoise(m_sigma, m_dTreeThreshold);
    ...
    bool renderSPP(Scene *scene, RenderQueue *queue, const RenderJob *job,
        ...
        while (result && m_passesRendered < nPasses) {
            ...
            /* @addition J.N
             * Switch to the full target density after enough training
             */
            if(m_denoising && m_iter >= 4 && 
                m_distributionStr == "switch" && 
                m_distribution == EDistribution::ESimple) {
                Log(EInfo, "SWITCHING TO FULL TARGET DENSITY");
                m_distribution = EDistribution::EFull;
            }
...
private:
    ...
    */
    ...
    /**
        @addition J.N
        Whether to reconstruct the guiding distributions or not
        Default = false
    */
    bool m_denoising;
    ...
    /**
        @addition J.N
        The standard deviation of the Gaussian filter used for denoising
        the guiding distributions
        Default = 0.8
    */
    Float m_sigma;
    ...
    /**
        @addition J.N
        Whether to attempt limiting the overhead of the quadtree reconstruction
        Default = true
    */
    bool m_recWL;
